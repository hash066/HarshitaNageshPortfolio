...
// Helper to parse data-edit-id to extract file path, line, and column
const parseEditId = (
  editId: string
...
export default function HoverReceiver() {
  ...
  const protectChildElements = (element: HTMLElement) => {
    ...
    childElements.forEach((child) => {
      ...
      childEl.setAttribute("data-edit-protected", "true");
  ...
  const restoreChildElements = (element: HTMLElement) => {
    ...
    protectedElements.forEach((child) => {
      ...
      childEl.removeAttribute("data-edit-protected");
  ...
  const handleTextChange = (element: HTMLElement) => {
    ...
    // Get the edit ID from the element to ensure we're working with the right one
  ...
  const handleStyleChange = (
    ...
    // Find ALL elements with the same edit ID
    ...
    const existingStyles = appliedStylesRef.current.get(editId) || {};
    appliedStylesRef.current.set(editId, { ...existingStyles, ...styles });
  ...
  const handleStyleBlur = (element: HTMLElement) => {
    ...
    const appliedStyles = appliedStylesRef.current.get(editId);
  ...
  const flushImageSrcChange = () => {
    ...
    const editId = imgElement.getAttribute("data-edit-id");
    if (!editId) return;
  ...
  useEffect(() => {
    ...
    const handleMouseUp = () => {
      if (focusedElementRef.current && focusedElementId) {
        ...
        // Extract file info from data-edit-id
        const editId = element.getAttribute("data-edit-id");
        if (editId) {
          const parsed = parseeditId(editId);
          if (parsed) {
  ...
  useEffect(() => {
    ...
    function onPointerMove(e: PointerEvent) {
      ...
      const hit =
        document
          ...
          ?.closest<HTMLElement>("[data-edit-id]") ?? null;
      ...
      if (hit !== lastHitElementRef.current) {
        ...
        const hitId = hit.getAttribute("data-edit-id");
        ...
        const tagName =
          hit.getAttribute("data-edit-name") || hit.tagName.toLowerCase();
    ...
    function onClickCapture(e: MouseEvent) {
      ...
      const target = e.target as HTMLElement;
      const hit = target.closest<HTMLElement>("[data-edit-id]");
      ...
      if (hit) {
        ...
        // Clicked on empty space or element without data-edit-id
      ...
      if (e.data.msg === "SHOW_ELEMENT_HOVER" && "elementId" in e.data) {
        ...
        if (allMatchingElements.length > 0) {
          ...
          allMatchingElements.forEach((element) => {
            ...
            if (!tagName) {
              tagName =
                element.getAttribute("data-edit-name") ||
